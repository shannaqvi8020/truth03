...
def download_model_with_progress(
    ...
    if progress_callback:
        progress_callback(0.02, f"Starting download of {model_name} model ({size_mb} MB)...")
    ...
    try:
            ...
            with open(temp_file, 'wb') as f:
                while True:
                    ...
                    if progress_callback:
                        ...
                        # Map download progress to 0.02 - 0.08 range
                        progress_val = 0.02 + (pct * 0.06)
                        progress_callback(
                            progress_val, 
        ...
        if progress_callback:
            progress_callback(0.08, f"Download complete! Loading model...")
...
def load_whisper_model(
    ...
    if not is_model_downloaded(model_name):
        ...
        if progress_callback:
            progress_callback(0.05, f"Loading {model_name} model...")
    
    if progress_callback:
        progress_callback(0.08, f"Loading model into {device_info['device_name']}...")
    ...
    if progress_callback:
        ...
        progress_callback(0.12, f"Model ready ({accel})")
...
def find_ffmpeg() -> str:
    ...
    if getattr(sys, 'frozen', False):
        ...
        # Check various bundled locations
        candidates.extend([
...
def extract_audio_with_ffmpeg(input_path: str, output_path: str) -> str:
    ...
    if not ffmpeg_cmd:
        ...
        if getattr(sys, 'frozen', False):
            exe_dir = os.path.dirname(sys.executable)
            raise RuntimeError(
                f"FFmpeg not found. Please place ffmpeg.exe in:\n"
                f"{os.path.join(exe_dir, 'ffmpeg_bin', 'ffmpeg.exe')}\n\n"
    ...
    try:
        # Build command - paths are passed as list items (properly escaped)
        cmd = [
            ffmpeg_cmd,
            '-i', input_path,
            '-vn',
        ...
        if sys.platform == 'win32':
            # Use shell=False (default) but set creation flags to hide window
            kwargs['creationflags'] = subprocess.CREATE_NO_WINDOW
            # Also set startupinfo for extra hiding
        ...
        raise RuntimeError(f"FFmpeg executable not found or invalid: {ffmpeg_cmd}")
    except subprocess.TimeoutExpired:
        raise RuntimeError("Audio extraction timed out (10 min limit).")
    except OSError as e:
        raise RuntimeError(f"OS error running FFmpeg: {e}")
...
def transcribe_audio(
    ...
    try:
        ...
        if is_video_file(audio_path):
            if progress_callback:
                progress_callback(0.18, "Extracting audio from video...")
        ...
        if progress_callback:
            progress_callback(0.22, "Starting transcription (this may take several minutes)...")
        ...
        if progress_callback:
            progress_callback(0.65, "Processing word-level confidence data...")
        ...
        if progress_callback:
            progress_callback(0.80, "Applying confidence thresholds...")
        ...
        if progress_callback:
            progress_callback(0.90, "Transcription complete!")
